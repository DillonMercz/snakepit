...
class Game {
    constructor() {
        ...
        // Game mode
        this.gameMode = 'classic'; // 'classic' or 'warfare'
    ...
    restart() {
        ...
        // Reset warfare mode properties
        this.weapons = [];
        this.projectiles = [];
        this.currentWeapon = null;
        this.lastShotTime = 0;
        
        // Let player select game mode again
        this.gameMode = null;
    }
    ...
    drawFood() {
        this.food.forEach(food => {
            ...
            if (x > -20 && x < this.canvas.width + 20 && y > -20 && y < this.canvas.height + 20) {
                ...
                // Draw food with enhanced appearance when in vacuum
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const warfareMode = document.getElementById('warfareMode');
    const warfareUI = document.querySelectorAll('#weapon, #cooldown, #warfareInstructions');
    ...
    function startGame(mode) {
        ...
        game.setupEventListeners(); // Set up mouse and keyboard controls
        game.gameLoop(); // Start the game loop

        // Show warfare UI elements if in warfare mode
        if (mode === 'warfare') {
            warfareUI.forEach(el => el.classList.remove('hidden'));
            document.body.style.cursor = 'crosshair';
        }
    }

    classicMode.addEventListener('click', () => startGame('classic'));
    warfareMode.addEventListener('click', () => startGame('warfare'));

    // Return to mode selection on game over
    document.getElementById('restartBtn').addEventListener('click', () => {
        document.getElementById('gameOver').classList.add('hidden');
        gameModeSelect.classList.remove('hidden');
        gameContainer.classList.add('hidden');
        warfareUI.forEach(el => el.classList.add('hidden'));
        if (game) {
